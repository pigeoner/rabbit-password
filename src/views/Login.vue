<template>
  <div class="login-box">
    <el-form>
      <el-form-item>
        <el-input size="large" v-model="mainPwd" :placeholder="localMainPwd ? '请输入主密码' : '请确定一个主密码'" show-password />
      </el-form-item>
      <el-form-item>
...
const login = () => {
  // 先加密
  const encryptedData = enc.encryptSHA512(mainPwd.value);
  if (localMainPwd) {
    /* 如果没有设置主密码 */
    // 密码输入错误
    if (encryptedData !== localMainPwd) return ElMessage.error('主密码错误');
    ...
    if (mainPwd.value?.length >= 6) {
      ...
      localStorage.setItem('mainPassword', encryptedData);
